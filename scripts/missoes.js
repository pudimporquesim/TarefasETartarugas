import { iniciarDialog, pegareventosdoarm2, today, iniciarToaster } from "./home.js";
export function iniciarmissoes() {
    ...
    btncriarmissao.addEventListener("click", () => {
        var caixa = document.querySelector("[data-dialog='form-missao']")
        const toaster = iniciarToaster(caixa);
        ...
        if (titulom.trim() === "") {
            toaster.error("O campo de nome de uma missão não pode ficar vazio.")
            return
        }
        if (datalm.trim() === "") {
            toaster.error("O campo de data limite de uma missão não pode ficar vazio.")
            return
        }
        if (dificuldadem == null) {
            toaster.error("O campo de dificuldade de uma missão não pode ficar vazio.")
            return
        }
        ...
        if (dataldata < hoje) {
            toaster.error("A data de uma missão não pode ser anterior a data de hoje.");
            return;
        }
        if (tarefas.length === 0) {
            toaster.error("Uma missão não pode ser criada sem tarefas.");
            return;
        };
        ...
        if (temTarefaInvalidaData.length > 0) {
            toaster.error("A data limite de uma tarefa de missão não pode ser posterior a data limite da missão.");
            return;
        };
        ...
        if (temTarefaInvalidaData2.length > 0) {
            toaster.error("A data de uma tarefa de missão não pode ser anterior a data de hoje.");
            return;
        };
        ...
        .done(function (data) {
            if (data.error != undefined) {
                ...
                setTimeout(() => {
                    document.dispatchEvent(new CustomEvent("missão-criada", {
                        bubbles: true
                   }));
                  }, 500);
              }
...
export async function aparecermissao() {
    ...
    if (ultimaData !== hoje.toDateString()) {
        ...
        if (tarefasatrasadas.length > 0 || missoesatrasadas.length > 0) {
            ...
            toaster.error(`Você acaba de receber ${danototal} de dano por conta de suas missões e tarefas atrasadas.`);
        }
...
function verseigual(titulomissao,desc,data,dataEvento2,missao,idmissao, mudancastarefas, tarefassembdjson) {
    let caixa = document.querySelector("[data-dialog='form-missao2']");
    const toaster = iniciarToaster(caixa);
    ...
    if (data) {
        ...
        if (data.val().trim() === "") {
            ...
            toaster.error("A data limite de uma missão não pode ser anterior ao dia de hoje.");
            return
        } 
    }
    ...
    if (temTarefaInvalida2) {
        toaster.error("O campo de data de uma tarefa não pode ficar vazio.");
        return;
    }
    ...
    async function atualiazarmissaobd(mudanca, mudancastarefas, tarefassembdjson) {
        let caixa = document.querySelector("[data-dialog='form-missao2']");
        const toaster = iniciarToaster(caixa);
        $.post("php/atualizarmissao.php", {mudanca, mudancastarefas, tarefassembdjson})
            .done(function (data) {
                if (data.error != undefined) {
                    toaster.error(data.error);
                } else if (data.success != undefined) {
                    toaster.success(data.success);
                    setTimeout(() => {
                        document.dispatchEvent(new CustomEvent("missão-atualizada/deletada", {
                            bubbles: true
                        }));
                      }, 500);
